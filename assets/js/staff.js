...
const STAFF_RIGHT = 680;
const NOTE_SPAWN_OFFSET = 44;
const NOTE_EXIT_OFFSET = 52;

export const TARGET_NOTE_PROGRESS =
  (STAFF_RIGHT + NOTE_SPAWN_OFFSET - (STAFF_LEFT + STAFF_RIGHT) / 2) /
  (STAFF_RIGHT + NOTE_SPAWN_OFFSET - (STAFF_LEFT - NOTE_EXIT_OFFSET));
...
export function renderStaff(svg, roundState) {
  ...
  const note = parseNote(noteId);
  const staffLeft = STAFF_LEFT;
  const staffRight = STAFF_RIGHT;
  const spawnX = staffRight + NOTE_SPAWN_OFFSET;
  const timingX = (staffLeft + staffRight) / 2;
  const exitX = staffLeft - NOTE_EXIT_OFFSET;
  const clampedProgress = Math.max(0, Math.min(1, noteProgress));
  const noteX = spawnX + (exitX - spawnX) * clampedProgress;
  const bottomY = 178;
  const stepHeight = 14;
  ...
  const toneMap = {
    neutral: {
      ...
      noteFill: "#1d1713",
      noteStroke: "#1d1713",
      glow: "rgba(181, 82, 45, 0.18)",
    },
    active: {
      bar: "#d0682f",
      noteFill: "#241b16",
      noteStroke: "#241b16",
      glow: "rgba(208, 104, 47, 0.26)",
    },
    success: {
      bar: "#1f7a52",
      noteFill: "#15563a",
      noteStroke: "#15563a",
      glow: "rgba(31, 122, 82, 0.24)",
    },
    error: {
      bar: "#ab2f2f",
      noteFill: "#8a2626",
      noteStroke: "#8a2626",
      glow: "rgba(171, 47, 47, 0.22)",
    },
  };
  const toneStyle = toneMap[tone] || toneMap.neutral;
  const noteScale = phase === "resolved" ? 1.14 : phase === "answering" ? 1.08 : 1;
  ...
  svg.append(
    createSvgElement("line", {
      ...
      x2: timingX,
      y2: 194,
      stroke: toneStyle.bar,
      "stroke-width": 4,
      opacity: 0.9,
    }),
  );

  if (showTargetGlow || phase === "answering" || phase === "resolved") {
    svg.append(
  ...
  noteGroup.append(
    createSvgElement("line", {
      x1: stemDirection === 1 ? noteX + 12 * noteScale : noteX - 12 * noteScale,
      y1: noteY,
      x2: stemDirection === 1 ? noteX + 12 * noteScale : noteX - 12 * noteScale,
      y2: noteY - stemDirection * 54 * noteScale,
      stroke: toneStyle.noteStroke,
      "stroke-width": 3,
      "stroke-linecap": "round",
    }),
